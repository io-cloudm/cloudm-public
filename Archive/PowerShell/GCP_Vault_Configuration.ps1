...
Function Configure-GCP-For-Migrate ([string]$ProjectId, [string]$ServiceAccountId, [string]$OutputPath = "C:\CloudM\GCPVaultConfig")
{
    ...
    if($ProjectNumber) {
        ...
        if($ServiceAccountClientId) {
            ...
            Write-Log $LogPath "Step 1. Configure Google Workspace Domain Wide Delegation using the following ClientId and Scopes:" $true
            Write-Host ""
            ...
            Write-Log $LogPath "Step 2. Service Account details for use in CloudM Archive Vault:" $true
